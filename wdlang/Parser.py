...
class WdReader:
  def __init__(self, file : str | pathlib.Path, wd_vars : dict = {}, presets : dict = {}) -> dict:
    ...
    # simboly one, simboly two, type data, function
    ...
    self.sections_simbolys  = {
                        ...
                        "wd_vars" : ("-|", "|-", 0, None, self.wd_vars_func),
                        "presets" : (".|", "|.", 0, self.presets_func, None),
                        "widgets" : ("<|", "|>", 0, self.widgets_func, None)
                      }
  ...
  def get_section(self, text : list, section_simbolys : dict) -> dict:
    ...
    for name_section, ss_td in section_simbolys.items():
      ...
      type_data       = ss_td[2]
      function_section= ss_td[3]
      function_data   = ss_td[4]
      ...
      for i in self.get_text_inside(text, *simboly_section):
        if type_data != None:
          ret[name_section].append(self.get_segments(i["text"], name_section, type_data, function_data))
        else:
      ...
      if function_section:
        function_section(ret[name_section])

      self.clear_text(text, rm[::-1], len(simboly_section[0]))

    return ret
  
  def get_segments(self, text : list, name_of_section : str, type_data : int, function_data,  simbolys_segments : dict = {}) -> dict:
    ...
    data_lines_values = self.get_data_line(text, type_data, function_data)
  ...
  def get_data_line(self, text : list, type_data : int, function_data) -> dict:
    ret : dict = {}
    start, end = None, None
    ...
    def name_value(line_text : str):
      ...
      if type_data == 0:
        ...
        val_ret = {"args" : args, "kwargs" : kwargs}

      if function_data:
        function_data({name : val_ret})
  ...
  def str_to_py(self, text : str):
    ...
    vglobals  : dict  = dict(self.wd_vars)
    unrm_text : str   = text
    rmval     : int   = 0
